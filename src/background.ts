...
import type { ActivePageContext, PageSnapshot, RestoreReport } from "./shared/types.js";
import { generateId, isSupportedUrl } from "./shared/utils.js";

type RuntimeMessage =
  | { type: "GET_ACTIVE_CONTEXT" }
  | { type: "LIST_SNAPSHOTS" }
  | { type: "SAVE_SNAPSHOT"; tabId: number; title: string; tags: string[] }
...
chrome.runtime.onMessage.addListener((message: RuntimeMessage, _sender, sendResponse) => {
  (async () => {
    if (message.type === "GET_ACTIVE_CONTEXT") {
      const tab = await getActiveTab();
      sendResponse({ ok: true, data: toContext(tab) } satisfies RuntimeResponse<ActivePageContext>);
      return;
    }

    if (message.type === "LIST_SNAPSHOTS") {
      const snapshots = await loadSnapshots();
      sendResponse({ ok: true, data: snapshots } satisfies RuntimeResponse<PageSnapshot[]>);
      return;
    }
    ...
    if (message.type === "SAVE_SNAPSHOT") {
      ...
      sendResponse({ ok: true, data: snapshot } satisfies RuntimeResponse<PageSnapshot>);
      return;
    }
    ...
    if (message.type === "RESTORE_SNAPSHOT") {
      ...
      if (!tabUrl || tabUrl.origin !== message.snapshot.origin) {
        sendResponse({ ok: false, error: "Snapshot origin does not match current tab origin." } satisfies RuntimeResponse<RestoreReport>);
        return;
      }
      ...
      sendResponse({ ok: true, data: result.result as RestoreReport } satisfies RuntimeResponse<RestoreReport>);
      return;
    }
  })().catch((error: unknown) => {
